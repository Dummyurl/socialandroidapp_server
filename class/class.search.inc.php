...
class search extends db_connect
{
    ...
    public function preload($itemId = 0, $gender = -1, $online = -1, $photo = -1)
    {
        if ($itemId == 0) {
    ...
    public function communitiesPreload($itemId = 0)
    {
        if ($itemId == 0) {
        ...
        $stmt = $this->db->prepare("SELECT id, regtime FROM users WHERE state = 0 AND account_type > 0 AND id < (:itemId) ORDER BY regtime DESC LIMIT 20");
