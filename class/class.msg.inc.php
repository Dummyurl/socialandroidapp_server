...
class msg extends db_connect
{
    ...
    public function create($toUserId, $chatId,  $message = "", $imgUrl = "", $chatFromUserId = 0, $chatToUserId = 0, $deviceId = "", $listId = 0, $ios_deviceId = "", $stickerId = 0, $stickerImgUrl = "", $android_msg_deviceId = "", $ios_msg_deviceId = "")
    {
        ...
        if ($stmt->execute()) {
            ...
            $msgInfo = array("error" => false,
                            ...
                            "fromUserState" => 0, // not used now
                            "fromUserVerify" => 0, // not used now
                            "fromUserUsername" => "", // not used now
                            "fromUserFullname" => "", // not used now
                            "fromUserPhotoUrl" => "", // not used now
            ...
            if (strlen($ios_deviceId) != 0) {
                ...
                $gcm->addDeviceId($ios_deviceId);
            }

            if (strlen($android_msg_deviceId) != 0) {

                $gcm->addDeviceId($android_msg_deviceId);
            }

            if (strlen($ios_msg_deviceId) != 0) {

                $gcm->addDeviceId($ios_msg_deviceId);
            }
        ...
        if ($stmt->execute()) {
            ...
            while ($row = $stmt->fetch()) {
                ...
                $chatInfo = array("error" => false,
                                  ...
                                  "with_android_msg_fcm_regId" => $profileInfo['android_msg_fcm_regid'],
                                  "with_ios_msg_fcm_regId" => $profileInfo['ios_msg_fcm_regid']);
